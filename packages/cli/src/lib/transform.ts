...
export function transformImports(
  ...
  function rewriteImport(
    ...
    // @onlynative/utils → local barrel
    if (importPath === '@onlynative/utils') {
      return `${prefix}${quote}${libAlias}/onlynative-utils${quote}`
    }

    // @onlynative/core → keep as-is (npm package)
    if (importPath.startsWith('@onlynative/core')) {
      return match
    }
    ...
    if (importPath.startsWith('../')) {
      ...
      if (
        targetComponent &&
        installedComponents.includes(targetComponent)
      ) {
  ...
  let result = source.replace(
    MULTI_LINE_IMPORT_REGEX,
    rewriteImport,
  )
  result = result.replace(
    SINGLE_LINE_IMPORT_REGEX,
    rewriteImport,
  )
...
export function generateUtilsBarrel(
  ...
  for (const utilName of utilNames.sort()) {
    ...
    if (exports && exports.length > 0) {
      lines.push(
        `export { ${exports.join(', ')} } from './${utilName}'`,
      )
    }
  }
